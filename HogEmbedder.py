...
class HogEmbedder(Embedder):
    ...
    def embedd(self, url) -> np.ndarray:
        if self.method == "HOG":
    ...
    def __init__(self, method="HOG"):
        self.method = method
        if self.method == "HOG":
            self._imgShape = (400,400)
            assert self._imgShape[0] % self.hogParams['pixels_per_cell'][0] == 0 and self._imgShape[1] % self.hogParams['pixels_per_cell'][1] == 0, "Shape and cell size need to be multiples"
            blocks_x = (self._imgShape[0] / self.hogParams['pixels_per_cell'][0]) - self.hogParams['cells_per_block'][0] + 1
            blocks_y = (self._imgShape[1] / self.hogParams['pixels_per_cell'][1]) - self.hogParams['cells_per_block'][1] + 1
